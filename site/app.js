...
$("#verifyBtn").addEventListener("click",async()=>{
  ...
  try{
    await window.AZWO_DATA?.createVerification?.({
      input_type:mode,
      input_text:mode==="text"?textarea.value:null,
      input_url:mode==="link"?$("#linkInput")?.value:null,
      domain:$("#domainSelect")?.value||"عام",
      language:$("#langSelect")?.value||"العربية"
    });
  }catch(err){
    console.warn("AZWO data layer:",err);
  }
...
$("#downloadReport").addEventListener("click",()=>{
  const payload={
    product:"AZWO",
    tagline:"تأصيل المعرفة",
    mode:window.AZWO_DATA?.mode||"demo",
  ...
  a.download="azwo-demo-report.json";
  a.click();
